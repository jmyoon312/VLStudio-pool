...
class UnicodeProcessor:
    ...
    def _preprocess_text(self, text: str, lang: str) -> str:
        ...
        if lang not in AVAILABLE_LANGS:
            # Fallback to English if lang is not supported, or raise error
    ...
    def __call__(
        ...
        for i, text in enumerate(text_list):
            ...
            # Robust Indexer Handling (Dict vs List)
            if isinstance(self.indexer, dict):
                # Legacy / Dict format
                seq = [self.indexer.get(str(val), 0) for val in unicode_vals]
            else:
                # Dense List Format (tokenizer.json is [token_id] * 65536)
                # Ensure val is within bounds
                seq = []
                for val in unicode_vals:
                    if val < len(self.indexer):
                        tid = self.indexer[val]
                        if tid == -1: tid = 0 # Map -1 to 0 (pad/unknown)
                        seq.append(tid)
                    else:
                        seq.append(0)
                        
            text_ids[i, : len(unicode_vals)] = np.array(seq, dtype=np.int64)
