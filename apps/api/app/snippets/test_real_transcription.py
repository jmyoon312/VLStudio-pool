...
def test_actual_transcription():
    print("🔍 Testing actual Whisper transcription...")
    ...
    try:
        ...
        if result['status'] == 'success':
            print(f"✅ Success! Transcription took {end-start:.2f}s")
            print(f"   Language: {result['language']}")
            print(f"   SRT Path: {result['srt_path']}")
            
            if os.path.exists(result['srt_path']):
                print(f"   ✅ SRT file verified on disk: {result['srt_path']}")
                ...
                print("   ❌ SRT file NOT found on disk!")
        else:
