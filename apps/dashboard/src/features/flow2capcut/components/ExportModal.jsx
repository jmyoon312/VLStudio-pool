    ...
    <div className="export-modal-overlay" onClick={loading ? undefined : onClose}>
      <div className="export-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="export-modal-content">
          ...
          {/* 생성될 경로 미리보기 + 프리셋 선택 */}
          {fullPath && (
            <div className={`path-preview-card ${pathPreset === 'custom' || pathManuallyEdited ? 'accent-custom' : 'accent-success'}`}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <label className="path-preview-label">
              ...
              <div style={{ display: 'flex', gap: '4px', marginTop: '6px', flexWrap: 'wrap' }}>
                {currentPresets.map(preset => (
                  <button
                    ...
                    className={`path-preset-btn ${pathPreset === preset.value ? 'active' : 'inactive'}`}
                  >
          ...
          {pathPreset === 'custom' && (
            <div className="path-guide-card">
              <p className="path-guide-card-title">
            ...
            {kenBurns && (
              <div style={{ marginLeft: '24px', marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }} title={t('exportModal.kenBurnsCycleTooltip')} className="kb-param-label">
                    <span className="kb-param-span">{t('exportModal.kenBurnsCycle')}</span>
                    <input
                      type="number"
                      min="1"
                      max="30"
                      ...
                      className="kb-number-input"
                    />
                    <span className="kb-param-span">{t('exportModal.kenBurnsCycleUnit')}</span>
                  </div>
                </div>
                {/* 스케일 범위 입력 */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }} title={t('exportModal.kenBurnsScaleTooltip')} className="kb-param-label">
                  <span className="kb-param-span">🔍 {t('exportModal.kenBurnsScale')}</span>
                  <input
                    type="number"
                    min="100"
                    max="150"
                    ...
                    className="kb-number-input"
                  />
                  <span className="kb-param-span">~</span>
                  <input
                    type="number"
                    min="100"
                    max="150"
          ...
          <div className="export-info">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              ...
              <button
                type="button"
                onClick={() => window.open('https://touchizen.github.io/guide/ko/viraloop/capcut-export.html', '_blank')}
                style={{
                  padding: '4px 10px',
                  fontSize: '0.8em',
                  background: '#007AFF',
